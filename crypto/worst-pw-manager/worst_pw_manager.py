...
def generate_key():
    # TODO: increase key length for more security?
    key = [KeyByteHolder(0)] * 8
    # use top secret master password to encrypt all passwords
    for i, c in enumerate(take(flag, 8)):
        key[i].num = c
    return key
...
def main(args):
    ...
    if args[1] == "import":
        ...
        for pw_idx, password in enumerate(passwords.splitlines()):
            ...
            with open(
                    "passwords/" + str(pw_idx) + "_" + masked_file_name +
                    ".enc", "wb") as f:
                f.write(rc4(password, generate_key()))
        print(
            "Import complete! Passwords securely stored on disk with your private key in flag.txt! You may now safely delete flag.txt."
        )
    else:
